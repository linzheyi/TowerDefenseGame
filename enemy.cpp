...
Enemy::Enemy(Road * startWayPoint,WoodMap *game,QString path):
    QObject(0),
    m_game(game),
    ...
    m_path(path)
{
    ...
    m_walkingSpeed=1;
    m_active=false;
    m_destinationRoadPoint=startWayPoint->getNextRoadPoint();//这里就充分利用了航点的好处，我们可以直接从当前航点得到下一个航点
}

Enemy::~Enemy()
{
    m_attackerTowerList.clear();//清楚在攻击该敌人的防御塔
    m_destinationRoadPoint=NULL;//全部设为空指针
    m_game=NULL;
}
...
void Enemy::move()
{
    ...
    if(collisionWithCircle(m_pos,1,m_destinationRoadPoint->getPos(),1))//如果到达了目标航点
    {
        if(m_destinationRoadPoint->getNextRoadPoint())//如果还存在下一个航点
        {//重新设置敌人的位置，和目标航点
            m_pos=m_destinationRoadPoint->getPos();
            m_destinationRoadPoint=m_destinationRoadPoint->getNextRoadPoint();
        }
    ...
    {//采用QVectoer2D中的两点移动方法
        QPoint targetPoint=m_destinationRoadPoint->getPos();
